...
void main()
{
	load_DFA_table(); //dfa 읽어서 출력
	print_dfa_table(DFAtab, N_DFA_states, N_symbols, DFA_finals);

	N_optDFA_states = optimize_DFA(DFAtab, N_DFA_states,
		N_symbols, DFA_finals, StateName, OptDFA); //optimize
	get_NEW_finals(NEW_finals, DFA_finals, StateName, N_optDFA_states);//final state찾기
